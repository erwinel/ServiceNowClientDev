...
function expectNullStatus(expected: any|null, result: any|null, message?: string): result is null {
    if (expected == null) {
        expect(result == null).toBe(true, message);
        return true;
    }
    expect(result == null).toBe(false, message);
    return false;
}
...
interface ModuleRegexPattern {
    name: string;
    getRegex: { (obj: x_44813_util.IJsTypeCommanderRegex): RegExp|undefined; };
...
describe("Testing regular expressions", function() {
    ...
    let regexOptionResult: x_44813_util.IJsTypeCommanderRegex = (function() {
        let r: x_44813_util.IJsTypeCommanderRegex|undefined;
