...
function expectNullStatus(expected: any|null, result: any|null, message?: string): result is null {
    if (expected == null) {
        expect(result).toBe(null, message);
        return true;
    }
    expect(result).not.toBe(null, message);
    return false;
}
...
interface ModuleRegexPattern {
    name: string;
    getRegex: { (obj: x_44813_util.JsTypeCommander.IJsTypeCommanderRegex|undefined): RegExp|undefined; };
...
describe("Testing regular expressions", function() {
    let patternDefinitions: ModuleRegexPattern[] = [
        {
            name: 'onlyWhitespace', getRegex: regexOptions => (x_44813_util.JsTypeCommander.notDefined(regexOptions)) ? undefined : regexOptions.onlyWhitespace, tests: [
            ...
            name: 'trimStart', getRegex: regexOptions => (x_44813_util.JsTypeCommander.notDefined(regexOptions)) ? undefined : regexOptions.trimStart, tests: [
            ...
            name: 'trimEnd', getRegex: regexOptions => (x_44813_util.JsTypeCommander.notDefined(regexOptions)) ? undefined : regexOptions.trimEnd, tests: [
                { input: "", expected: false }, { input: " \t\r\n ", expected: false }, { input: " \t\r\n ", expected: false },
            ...
            name: 'lineSeparator', getRegex: regexOptions => (x_44813_util.JsTypeCommander.notDefined(regexOptions)) ? undefined : regexOptions.lineSeparator, tests: [
            ...
            name: 'booleanText', getRegex: regexOptions => (x_44813_util.JsTypeCommander.notDefined(regexOptions)) ? undefined : regexOptions.booleanText, tests: (function() {
            ...
            name: 'firstLetterLc', getRegex: regexOptions => (x_44813_util.JsTypeCommander.notDefined(regexOptions)) ? undefined : regexOptions.firstLetterLc, tests: [
            ...
            name: 'abnormalWhitespace', getRegex: regexOptions => (x_44813_util.JsTypeCommander.notDefined(regexOptions)) ? undefined : regexOptions.abnormalWhitespace, tests: [
                { input: "  ", expected: { captures: [], groupZero: "  " } }, { input: " \t\r\n ", expected: { captures: [], groupZero: " \t\r\n " } },
    ...
    let regexOptionResult: x_44813_util.JsTypeCommander.IJsTypeCommanderRegex|undefined = (function() {
        let r: x_44813_util.JsTypeCommander.IJsTypeCommanderRegex|undefined;
        try { r = x_44813_util.JsTypeCommander.getPatternOptions(); } catch { r = undefined; }
        return r;
    })();
                ...
                it('regexOptions.' + pattern.name + '.exec(' + JSON.stringify(testData.input) + ') should return ' + JSON.stringify(expectedGroups), function() {
                    ...
                    if (typeof(regex) == "undefined")
                        ...
                        if (expectNullStatus(expectedGroups, result, 'Unexpected match result') || expectedGroups === null)
                            return;
                        expect(result.length).toBe(expectedGroups.length, 'Length mismatch');
                        for (let i: number = 0; i < expectedGroups.length; i++) {
                            ...
                            if (typeof(expectedGroups[i]) == "string") {
                                expect(typeof(result[i])).toBe('string', msg);
                                expect(JSON.stringify(result[i])).toBe(JSON.stringify(expectedGroups[i]), msg);
                            } else
                                expect(typeof(result[i])).not.toBe('string', msg);
                        }
                    }
                });
            }, this);
